...
export default function SignupPage() {
  ...
  const [form, setForm] = useState({
    ...
    gender: '', dob: '', country: '', referralCode: refFromUrl,
  })
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    const { fullName, email, password, confirmPassword, gender, dob, country, referralCode } = form
    ...
    setLoading(true)
    try {
      await signUp({ email, password, fullName, gender, dob, country, avatarFile, referralCode: referralCode.trim() })
      setSuccess(true)
    } catch (err) {
      setError(
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-transparent">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          ...
          {form.referralCode && (
            <div className="bg-green-900/30 border border-green-500/30 rounded-xl p-3 text-sm text-green-300">
              🎉 Referral applied! Your referrer just earned <strong>{REFERRAL_BONUS} tokens</strong>.
            </div>
          )}
        ...
        <div className="bg-gray-900 rounded-2xl p-5 border border-gray-800 space-y-3">
          ...
          <div>
            ...
            <input
              ...
              maxLength={10}
              className={`w-full bg-gray-800 border rounded-xl px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none font-mono tracking-widest uppercase ${
                refFromUrl ? 'border-green-600 bg-green-900/20' : 'border-gray-700 focus:border-pink-500'
              }`}
            />
            {refFromUrl && <p className="text-xs text-green-400 mt-0.5">✅ Referral code applied automatically!</p>}
            {!refFromUrl && <p className="text-xs text-gray-500 mt-0.5">Your referrer earns {REFERRAL_BONUS} tokens when you join</p>}
          </div>
